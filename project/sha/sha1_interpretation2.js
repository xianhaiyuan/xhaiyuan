var hex_sha1 = function(){
    var hexcase = 0;  //初始化最后的输出结果为大写字母还是小写字母,0为小写，1为大写
    /*
     *
     * 主函数
     *
    ...
    function sha1_vm_test() {
        ...
        return hex_sha1("中文中文中文中文中文中文中文");
    ...
    function core_sha1(blockArray) {
        ...
        for (var i = 0; i < x.length; i += 16) // 每次处理512位 16*32
        {
            ...
            for (var j = 0; j < 80; j++) // 对每个512位进行80步操作
            {
                ...
                //从x数组中依次地每次将4*8=32比特（4个字符）的信息存到w数组里，即第二层的循环中(j的循环)每次拿4个字符的信息存到w数组里。
                ...
                //x[i]数组从前到后，第一次：w[0]=x[0],w[1]=x[1]...,w[15]=x[15];第二次：w[0]=x[16],w[1]=x[17]...,w[15]=x[31];即最外层的每次循环（x.length的循环)依次地拿512比特的str字符串信息存到w数组里
                    ...
                    //将w[j]的前3个数，前8个数，前14个数，前16个数做异或运算之后得到的值为num，将num传进rol函数做运算后得到的值，即为w[j]的值
                    //此处rol函数作用为循环左移，将第一个参数的值分别左移一位，右移31位再将这两个数做|运算，组成一个32位二进制数

                //safe_add作用为模2^32相加，此处为5个字以模2^32相加（循环左移了5个字）

                //a,b,c,d,e <- ( e+f(j,b,c,d)+rol(a,5)+w+k ),a,rol(b,30),c,d  （此处为压缩函数）

                var tmp = safe_add( safe_add(rol(a, 5), sha1_ft(j, b, c, d)), safe_add(safe_add(e, w[j]), sha1_kt(j)) );
    ...
    function binb2hex(binarray) {
        ...
        for (var i = 0; i < binarray.length * 4; i++) {
            ...
            //转换过程如下：第一个数为例，第一次循环binarray[i >> 2]表示i=0到3取的都是binarray[0]，binarray[0]==a，右式的第一项表示右移28位，于是取得32位中的前4位，转换为十进制后在hex_tab里找到的对应的十六进制数字；第二项表示右移24位，于是取得32位中的前8位，做& 0xF运算后去掉前面的位数，保留后4位，转换为十进制后在hex_tab里找到的对应的十六进制数字。一次循环得到2个十六进制数字，以此类推，20次循环最后得到40位十六进制数字，即160比特的二进制数据。
